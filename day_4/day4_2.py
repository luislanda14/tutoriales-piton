...
def intersects(listA: list, listB: list) -> bool:
    """Comprueba si la lista B se solapa de alguna manera con la lista A.
        La lista A debe tener más o los mismos elementos que la lista B
    """
    num = 0
    for a in listA:
        try:
...
def main(input_path):
    ...
    for line in datos:
        ...
        if len(elfB) > len(elfA):
            aux = elfB
            elfB = elfA
            elfA = aux

        if intersects(elfA, elfB):
            count += 1
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Script para Advent of Code Día 4 parte 2')
